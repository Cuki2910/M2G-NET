# TG-MVMT-GFNet v2: Theory-Guided Multi-View Multi-Task Gated Fusion Network
...
**Tên mô hình đề xuất:** **TG-MVMT-GFNet v2**
...
Mô hình **TG-MVMT-GFNet v2** được đề xuất để kiểm tra câu hỏi sau:
...
## 6. Mô hình đề xuất: TG-MVMT-GFNet v2
...
Level 4 — Regularized Task-Specific Gated Fusion:
    For each task t:
        α_t = softmax(Gate_t([h_role, h_trait, h_road, h_env, h_site, h_inter]) + temperature_prior)
...
# Dùng:
logits_t = W_gate_t @ concat(H_full) + prior
alpha_t   = softmax(logits_t / temperature)

# prior = vector khởi tạo đều để tránh mode collapse
...
TG-MVMT-GFNet v2 có thể bổ sung:
...
Rất mạnh với dữ liệu tabular. **Baseline quan trọng nhất về predictive performance**. Nếu TG-MVMT-GFNet không vượt được XGBoost về accuracy, cần argue rõ giá trị nằm ở view-level attribution và interpretability, không phải predictive superiority.
...
Với XGBoost/Random Forest, dùng SHAP hoặc permutation importance để so sánh chéo với view-level attribution của TG-MVMT-GFNet v2.
...
TG-MVMT-GFNet v2         xx       xx      xx   xx       xx
```
...
**TG-MVMT-GFNet v2: A Theory-Guided Multi-View Fusion Extension for Motorcycle Risky Behavior Prediction**
...
[ ] Có framework diagram của TG-MVMT-GFNet v2 (phân tầng individual–contextual)
...
**TG-MVMT-GFNet v2** là phiên bản cải thiện cân bằng được bốn yêu cầu:
